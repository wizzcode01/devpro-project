      ...
      <div className="flex flex-col gap-20">
        {blogPosts.map((post, idx) => (
          <div key={idx} className="grid grid-cols-4 gap-6">
            ...
            {/* Uses (single row, smaller span) */}
            <div className="col-span-1 flex flex-col gap-2 text-start text-white p-4">
              <h1 className="font-bold">Uses</h1>
              <h2 className="text-white font-bold">{post.uses.name}</h2>
              <p className="text-zinc-400">{post.uses.desc}</p>
            </div>
          </div>
        ))}
      </div>
